...
class PatientController {
    ...
    static getMyPatients(req, res) {
        if (req.user?.role !== 'dentist') {
            return res.status(403).json({ success: false, error: 'Only dentists have assigned patients' });
        }
        PatientService.getPatientsForUser(req.user, (err, patients) => {
            if (err) {
                return res.status(500).json({ success: false, error: err.message });
    ...
    static async getPatientById(req, res) {
        ...
        try {
            const patient = await PatientService.getPatientByIdForUser(id, req.user);
            if (!patient) {
                return res.status(404).json({ success: false, error: 'Patient not found' });
            }
            const { canAccessClinical } = require('../utils/clinicalAccess');
            patient.canViewClinical = await canAccessClinical(req.user, id);
            res.json({ success: true, data: patient });
        } catch (err) {
            res.status(500).json({ success: false, error: err.message });
        }
    }
    static getClinicalAccess(req, res) {
        const id = req.params.id;
        const { canAccessClinical } = require('../utils/clinicalAccess');
        canAccessClinical(req.user, id).then((allowed) => {
            res.json({ success: true, canAccess: allowed });
        }).catch((err) => {
            res.status(500).json({ success: false, error: err.message });
        });
    }
    ...
    static getPatientByMRN(req, res) {
        ...
        PatientService.getPatientByMRN(mrn, async (err, patient) => {
            if (err) {
                return res.status(500).json({ success: false, error: err.message });
            }
            if (!patient) {
            ...
            if (req.user?.role === 'dentist') {
                const scoped = await PatientService.getPatientByIdForUser(patient.id, req.user);
                if (!scoped) {
                    return res.status(404).json({ success: false, error: 'Patient not found' });
                }
            }
    ...
    PatientService.getPatientById(id, (err, existing) => {
        ...
        delete merged.registration_date;

        PatientService.updatePatient(id, merged, async (updateErr) => {
            if (updateErr) {
            ...
            publishChange('patient', 'updated', { id }, req.user?.id);
    ...
    static searchPatients(req, res) {
        ...
        PatientService.searchPatientsForUser(searchTerm, req.user, (err, patients) => {
            if (err) {
                return res.status(500).json({ success: false, error: err.message });
            }
            res.json({ success: true, data: patients });
    ...
    static async generatePatientPDF(req, res) {
        const id = req.params.id;
        try {
            const patient = await PatientService.getPatientByIdForUser(id, req.user);
            if (!patient) {
    ...
    static _streamPatientPdf(res, patient) {
    ...
    static generateAllPatientsPDF(req, res) {
        if (!canExportAllPatients(req.user)) {
    ...
    static exportToExcel(req, res) {
        ...
        PatientService.getPatientsForUser(req.user, async (err, patients) => {
            if (err) {
                return res.status(500).json({ success: false, error: err.message });
            }
