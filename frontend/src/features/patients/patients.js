...
export class Patients {
            ...
            <div class="patients-container">
                ...
                <div class="patients-controls">
                    ...
                    <div class="export-buttons">
                        ${canAssignDoctors() ? `
                        <button class="export-btn assign-doctors" id="bulkAssignDoctorsBtn" title="Auto-assign available dentists to unassigned patients">
    ...
    updateTable() {
        ...
        tbody.innerHTML = pagePatients.map(patient => `
            <tr data-patient-id="${patient.id}" class="${this.selectedPatients.has(patient.id) ? 'selected' : ''}">
                ...
                <td>${this.renderDoctorCell(patient)}</td>
                <td>
                    <div class="action-buttons-group">
                        ${canAssignDoctors() && !patient.assigned_doctor_id ? `
                        <button class="action-btn assign" data-id="${patient.id}" title="Auto-assign doctor">
                            <i class="fas fa-user-md"></i>
                        </button>
                        ` : ''}
    ...
    setupEventListeners() {
        ...
        document.getElementById('bulkAssignDoctorsBtn')?.addEventListener('click', () => this.bulkAssignDoctors());
    ...
    renderDoctorCell(patient) {
        if (!patient.assigned_doctor_id) {
            return '<span class="doctor-cell none">Not assigned</span>';
        }
        const status = patient.assignment_status === 'confirmed' ? 'confirmed' : 'pending';
        const icon = status === 'confirmed' ? 'fa-check-circle' : 'fa-clock';
        return `
            <span class="doctor-cell ${status}">
                <i class="fas ${icon}"></i>
                ${this.escapeHtml(patient.assigned_doctor_name || 'Doctor')}
            </span>`;
    }

    async assignDoctorToPatient(patientId) {
        try {
            const result = await appointmentService.assignDoctor({ patientId, auto: true });
            if (window.Toast) {
                window.Toast.success(`${result.staffName} assigned — awaiting confirmation`);
            }
            await this.loadPatients();
        } catch (err) {
            if (window.Toast) window.Toast.error(err.message || 'Assignment failed');
        }
    }

    async bulkAssignDoctors() {
        if (!confirm('Auto-assign available dentists to all unassigned patients? Each gets the next free slot.')) return;
        try {
            const data = await appointmentService.bulkAssignDoctors();
            const count = data.assigned?.length || 0;
            if (window.Toast) {
                window.Toast.success(`Assigned ${count} patient(s) to dentists`);
            }
            if (data.errors?.length && window.Toast) {
                window.Toast.warning(`${data.errors.length} could not be assigned (no slots)`);
            }
            await this.loadPatients();
        } catch (err) {
            if (window.Toast) window.Toast.error(err.message || 'Bulk assign failed');
        }
    }
